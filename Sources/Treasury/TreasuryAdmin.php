...
function treasuryTotals()
{
	...
	$searchtime = $search_time == '0' ? '0' : mktime() - ($search_time * 86400);
	$start = isset($_POST['start']) ? intval($_POST['start']) : (isset($_GET['start']) ? intval($_GET['start']) : 0);
	$sort_order = isset($_POST['order']) ? $_POST['order'] : (isset($_GET['order']) ? $_GET['order'] :'');
	$sort_order = (isset($sort_order) && $sort_order == 'ASC') ? 'ASC' : 'DESC';
...
#		$custom = htmlentities($_POST['Custom'], ENT_QUOTES);
		...
		$insert_Transaction = "
			...
			VALUES 
			(DEFAULT, '$user_id', '$bizness', '$itemname', '$itemnum', '$_POST[Txn_id]', '$_POST[Custom]', '$_POST[Option_seleczion1]', '$_POST[Payment_status]', '$_POST[Mc_currency]', '$currency_symbol', '$_POST[Mc_gross]', '$_POST[Mc_fee]', '$_POST[Settle_amount]', '$exchange_rate', 'web_accept', '$nTime', '$_POST[Eid]')";
...
function eventsEdit()
{
	...
	{
		$sTime = strtotime($_POST['date_start']) + mktime(0, 0, 0, 1, 1, 1970);
		$eTime = empty($_POST['date_end']) ? '0' : strtotime($_POST['date_end']) + mktime(0, 0, 0, 1, 1, 1970);
		$insert_Recordset = "
