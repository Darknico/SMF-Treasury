...
$tr_period[2] = $duration == 0 ? strftime('%B', forum_time()) : ($duration == 1 ? $txt['treas_quarterly'] : ($duration == 2 ? $txt['treas_half_yearly'] : $txt['treas_year'].' '.$year));
$tr_period[3] = 0;
if (!$duration) {
...
if ($tr_config['event_active'] && isset($context['info_event']))
{
	...
	{
		$row_Recordset2['due_by'] = !empty($event_info['date_end']) ? timeformat($event_info['date_end'], '%b %d') : $txt['treas_event_open'];
